	...
	<view class="container">
		...
		<view class="section">
			...
			<view class="status-box">
				...
				<text class="status-value" :class="{ 'active': isListeningWeight }">{{ isListeningWeight ? "已开启" : "未开启" }}</text>
			</view>
			<view class="log-box">
		...
		<view class="section">
			...
			<view class="status-box">
				<text class="status-label">监听状态:</text>
				<text class="status-value" :class="{ 'active': isListeningTranslation }">{{ isListeningTranslation ? "已开启" : "未开启" }}</text>
			</view>
			<view class="log-box">
		...
		<view class="tips">
			...
			<view class="tips-content">
				...
				<text class="tips-item">4. wx.onUserTriggerTranslation: 监听用户触发翻译事件</text>
				<text class="tips-item">5. wx.offUserTriggerTranslation: 取消监听用户触发翻译事件</text>
			</view>
		</view>
	</view>
</template>
...
const menuButtonWeightChangeCallback = (res) => {
	...
	// 确保 res 对象存在且包含 width 属性
	if (res && res.width !== undefined) {
		weightChangeLogs.value.push(`[${timeString}] 宽度变为: ${res.width}`);
	} else {
...
const userTriggerTranslationCallback = (res) => {
	...
	if (res && res.locale) {
		translationLogs.value.push(`[${timeString}] 翻译到的目标语言: ${res.locale}`);
	} else {
...
const onTranslation = () => {
	...
	try {
		wx.onUserTriggerTranslation(userTriggerTranslationCallback);
...
const offTranslation = () => {
	...
	try {
		wx.offUserTriggerTranslation(userTriggerTranslationCallback);
