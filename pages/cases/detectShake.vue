...
const lastShakeTime = ref(null);
const shakeStatus = ref("未检测");
const accelerometerData = ref({ x: 0, y: 0, z: 0 });
const lastAccelerometer = ref({ x: 0, y: 0, z: 0 });
let shakeTimer = null;

// 开始监听加速度
const startListen = () => {
	isListening.value = true;
	shakeStatus.value = "监听中...";

	// 开启加速度监听
	uni.onAccelerometerChange((res) => {
		accelerometerData.value = res;
		detectShake(res);
	});
...
const detectShake = (currentData) => {
	// 计算三轴加速度差值
	const deltaX = Math.abs(currentData.x - lastAccelerometer.value.x);
	const deltaY = Math.abs(currentData.y - lastAccelerometer.value.y);
	const deltaZ = Math.abs(currentData.z - lastAccelerometer.value.z);

	// 保存当前值为上一次的值
	lastAccelerometer.value = { ...currentData };
